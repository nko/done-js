...
ResourceManager.prototype.handleWSRequest = function(conn, msg) {
  ...
  } else if ('revoke-share' == req['request']) {
    var url = req['url']
    for (i = 0; i < this.resources.length; i++) {
      var r = resources[i];
      if (r.url == url) {
	conn.send(JSON.stringify({'response' : 'revoked',
				  'name' : r.name,
				  'url' : r.url}));
...
function Resource(wsconn, base, token, name, size, type) {
  var self = this;
  this.wsconn = wsconn;
  this.url = base + ResourceManager.CONTEXT + '/' +
    token + "/" + urlencode.encode(name);
  this.token = token;
  this.name = name;
  this.size = size;
  this.type = type;
